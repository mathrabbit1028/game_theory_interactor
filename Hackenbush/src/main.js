...
function edgeMouseOverFunc(graph){
    ...
    d3.selectAll(".links path")
        ...
        .style("stroke", (document.getElementById("p1").style.color === "red" ? "red" : "blue"));
    };
}
...
function gameOverScreen(turn){
    if (turn % 2) document.getElementById("message").textContent = "Player 1(선) win!";
    else document.getElementById("message").textContent = "Player 2(후) win!";
...
for (let i = 0; i < levelBtn.length; i++) {
    levelBtn[i].onclick = function (e) {
        selectLevel(i+1);
    }
}

function main(){
    fetch("./static/graph"+level+".json")
        .then(r => r.json())
        .then(G => runGame(G))
}
